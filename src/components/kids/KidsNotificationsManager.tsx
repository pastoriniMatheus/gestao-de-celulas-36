...
export function KidsNotificationsManager() {
  ...
  const kidsMinistries = ministries.filter(m => 
    m.name.toLowerCase().includes('kids') || m.name.toLowerCase().includes('jovens')
  );
        ...
        <Card className="bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-200">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-blue-700">
              ...
              Ministérios Kids e Jovens
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {kidsMinistries.map(ministry => (
                <Card key={ministry.id} className="bg-white">
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      ...
                      <div className="space-y-1">
                        ...
                        {ministryTeachers
                          .filter(teacher => teacher.ministry_id === ministry.id)
                          .map(teacher => (
                            <div key={teacher.id} className="text-xs bg-gray-100 px-2 py-1 rounded">
                              {teacher.contact.name} ({teacher.teacher_type === 'teacher_1' ? 'Professora 1' : 'Professora 2'})
                            </div>
                          ))}
                        {ministryTeachers.filter(teacher => teacher.ministry_id === ministry.id).length === 0 && (
                          <p className="text-xs text-gray-500">Nenhuma professora cadastrada</p>
                        )}
                      </div>
                    </div>
                  </CardContent>
